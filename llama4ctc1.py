...
def train():
    ...
    logger.info(
        "基于Lora 4-bit微调llama2实现CTC任务，超参设置--mode train --num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d --train_path %s --model_path %s" % (
...
def retrain():
    ...
    logger.info(
        "基于Lora 4-bit继续微调llama2实现CTC任务，超参设置--mode retrain --num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d --train_path %s --model_path %s --model_checkpoint %s" % (
            pargs.num_samples, pargs.max_length, pargs.num_epochs, pargs.lr, pargs.batch_size, pargs.accum_steps,
            pargs.train_path, pargs.model_path, pargs.model_checkpoint))

    logger.info("开始创建分词器...")

    prompt_config = LoraConfig.from_pretrained(pargs.model_path + pargs.model_checkpoint)
...
def generator():
    ...
    logger.info(
        "基于Lora 4-bit调用llama2微调模型实现CTC任务，超参设置--mode infer --max_new_tokens %d --model_path %s --model_checkpoint %s" % (
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", default="models/llama4ctc1/", type=str)
