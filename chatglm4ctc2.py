...
def retrain(checkpoint):
    logger = create_logger(name="retrain_log",
                           filename=checkpoint + "/chatglm4ctc2.log")
    logger.info(
        "------------------------------------------------------------------------------------------------------------------------------------------")

    logger.info("RETRAIN LOGGING......")
    logger.info(
        "基于QLora 4-bit继续微调chatglm3实现CTC任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d" % (
            pargs.num_samples, pargs.max_length, pargs.num_epochs, pargs.lr, pargs.batch_size, pargs.accum_steps))
...
def generator(checkpoint):
    ...
    logger.info(
        "基于QLora 4-bit调用chatglm3微调模型实现CTC任务，超参数有--max_new_tokens %d" % (pargs.max_new_tokens))
...
if __name__ == "__main__":
    ...
    if pargs.mode == "train":
        ...
        retrain("models/chatglm4ctc2")
    elif pargs.mode == "infer":
        generator("models/chatglm4ctc2")
