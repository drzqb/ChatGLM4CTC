...
class MyDataset(Dataset):
    ...
    def __init__(self, data_path, tokenizer):
        prompt_prefix = "句子“"
        prompt_suffix = "”中由于拼音相近或者字形相近造成的错别字有："
                ...
                if originsen == correctsen:
                    ...
                    for res in words:
                        ...
                        if originword != correctword:
                            answer += str(k) + "." + originword + "：应为“" + correctword + "”。\n"
                            k += 1
                    answer += "修改后的句子为：“" + correctsen + "“"

                # print(prompt_prefix + originsen + prompt_suffix + answer)
...
def train():
    ...
    logger.info(
        "基于QLora 4-bit微调chatglm3实现CTC任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d --train_path %s --model_path %s" % (
...
def retrain():
    ...
    logger.info(
        "基于QLora 4-bit继续微调chatglm3实现CTC任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d --train_path %s --model_path %s --model_checkpoint %s" % (
            pargs.num_samples, pargs.max_length, pargs.num_epochs, pargs.lr, pargs.batch_size, pargs.accum_steps,
...
def generator():
    ...
    logger.info(
        "基于QLora 4-bit调用chatglm3微调模型实现CTC任务，超参数有--max_new_tokens %d --model_path %s --model_checkpoint %s" % (
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", default="models/chatglm4ctc4/", type=str)
