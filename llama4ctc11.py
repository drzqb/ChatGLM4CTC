'''
    基于Llama2完成文本纠错任务 Llama2-Chinese-7b-Chat-ms
    soft prompt -4bit + 半精度float16
'''
...
import warnings
import logging
from peft import get_peft_model, PromptTuningConfig, TaskType, PeftModel, prepare_model_for_kbit_training
...
def train():
    ...
    logger = create_logger(name="train_log",
                           filename=pargs.model_path + "/llama4ctc.log")
    logger.info(
        "------------------------------------------------------------------------------------------------------------------------------------------")
    ...
    logger.info(
        "基于SoftPrompt 4-bit微调llama2实现CTC任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d --train_path %s --model_path %s" % (
    ...
    pretrained_checkpoint = "D:/pythonwork/LLMs4NER/uer/Llama2-Chinese-7b-Chat-ms"
...
def retrain():
    ...
    logger.info(
        "基于SoftPrompt 4-bit继续微调llama2实现CTC任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d --train_path %s --model_path %s --model_checkpoint %s" % (
            pargs.num_samples, pargs.max_length, pargs.num_epochs, pargs.lr, pargs.batch_size, pargs.accum_steps,
...
def generator():
    ...
    logger.info(
        "基于SoftPrompt 4-bit调用llama2微调模型实现CTC任务，超参数有--max_new_tokens %d --model_path %s --model_checkpoint %s" % (
    ...
    while intent:
        ...
        # 方法二：使用generate函数
        prompt = '<s>[INST] <<SYS>>\n\n<</SYS>>\n\n' + query + ' [/INST]\n\n'
        res = tokenizer(prompt, add_special_tokens=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", default="models/llama4ctc11/", type=str)
